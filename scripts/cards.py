...
def icon(path,x,y,size,fill):
    s=size/16; return f'<path transform="translate({x:.1f},{y:.1f}) scale({s:.3f})" fill="{fill}" d="{path}"/>'
...
def fetch_contributions(user,token):
    ...
    days=sorted((dt.date.fromisoformat(d["date"]),d["contributionCount"]) for w in cal["weeks"] for d in w["contributionDays"])
    longest=run=0
...
def main(argv=None):
    ...
    while True:
        ...
        page+=1
    owned=[r for r in repos if not r["fork"]];stars=sum(r["stargazers_count"] for r in owned)
    # Deliberately keep the card to four useful fields: public repos, followers,
    # total contributions (1y), and longest streak. Stars/current streak are omitted.
    contrib=fetch_contributions(a.user,token); total=contrib[0] if contrib else "—"; longest=contrib[2] if contrib else "—"
    tiles=[("Public repos",f"{user['public_repos']:,}"),("Followers",f"{user['followers']:,}"),("Contributions (1y)",f"{total:,}" if isinstance(total,int) else total),("Longest streak",f"{longest:,}" if isinstance(longest,int) else longest)]
